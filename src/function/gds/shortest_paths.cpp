...
public:
    ...
    explicit PathLengths(std::vector<std::tuple<table_id_t, uint64_t>> nodeTableIDAndNumNodes)
        : curFrontierFixedTableID{INVALID_TABLE_ID}, curFrontierFixedMask{nullptr},
          nextFrontierFixedMask{nullptr} {
        for (const auto& [tableID, numNodes] : nodeTableIDAndNumNodes) {
