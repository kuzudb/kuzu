...
static void play(table_id_t relTableID, graph::Graph* graph, ExtendDirection extendDirection, RJCompState& rjCompState, processor::ExecutionContext* context) {
    auto info = FrontierTaskInfo(relTableID, graph, extendDirection,
    ...
    while (frontierPair->hasActiveNodesForNextLevel() && frontierPair->getNextIter() <= maxIters) {
        ...
        for (auto& relTableIDInfo : graph->getRelTableIDInfos()) {
            ...
            case ExtendDirection::FWD: {
                ...
                play(relTableIDInfo.relTableID, graph, common::ExtendDirection::FWD, rjCompState, context);
            } break;
            case ExtendDirection::BWD: {
                rjCompState.beginFrontierComputeBetweenTables(relTableIDInfo.toNodeTableID,
                    relTableIDInfo.fromNodeTableID);
                play(relTableIDInfo.relTableID, graph, common::ExtendDirection::BWD, rjCompState, context);
            } break;
