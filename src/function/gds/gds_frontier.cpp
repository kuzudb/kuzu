...
void LocalFrontierSample::addNodes(const std::vector<common::nodeID_t> nodeIDs) {
    if (!enabled_) {
        return;
    }
    for (auto nodeID : nodeIDs) {
        offsetSet.insert(nodeID.offset);
    }
}

void LocalFrontierSample::checkSampleSize() {
    if (!enabled_) {
        return;
    }
    enabled_ = offsetSet.size() < SAMPLE_SIZE;
}
...
void SparseFrontier::mergeLocalFrontier(const LocalFrontierSample& localFrontier) {
    std::unique_lock<std::mutex> lck{mtx};
...
void FrontierPair::mergeLocalFrontier(const LocalFrontierSample& localFrontier) {
    std::unique_lock<std::mutex> lck{mtx};
...
void SinglePathLengthsFrontierPair::beginFrontierComputeBetweenTables(table_id_t curTableID,
    ...
    curSparseFrontier->pinTableID(curTableID);
    nextSparseFrontier->pinTableID(nextTableID);
...
void DoublePathLengthsFrontierPair::beginFrontierComputeBetweenTables(table_id_t curTableID,
    table_id_t nextTableID) {
    curDenseFrontier->ptrCast<PathLengths>()->pinCurFrontierTableID(curTableID);
    nextDenseFrontier->ptrCast<PathLengths>()->pinNextFrontierTableID(nextTableID);
    curSparseFrontier->pinTableID(curTableID);
    nextSparseFrontier->pinTableID(nextTableID);
    morselDispatcher.init(curTableID, curDenseFrontier->getNumNodes(curTableID));
}
