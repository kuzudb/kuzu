...
static std::unique_ptr<TableFuncBindData> internalCreateHNSWBindFunc(main::ClientContext* context,
...
static offset_t createHNSWTableFunc(const TableFuncInput& input, TableFuncOutput&) {
...
static double internalCreateHNSWProgressFunc(TableFuncSharedState* sharedState) {
...
static std::unique_ptr<processor::PhysicalOperator> getPhysicalPlan(
    ...
    auto createHNSWFuncSharedState =
        createHNSWTableFuncCall->getSharedState()->funcState->ptrCast<CreateHNSWSharedState>();
...
function_set InternalCreateHNSWIndexFunction::getFunctionSet() {
    ...
    func->bindFunc = internalCreateHNSWBindFunc;
    func->initSharedStateFunc = initCreateHNSWSharedState;
    func->initLocalStateFunc = initCreateHNSWLocalState;
    func->tableFunc = createHNSWTableFunc;
    func->progressFunc = internalCreateHNSWProgressFunc;
...
static offset_t finalizeHNSWTableFunc(const TableFuncInput& input, TableFuncOutput&) {
    ...
    for (auto i = morsel.startOffset; i < morsel.endOffset; i++) {
        hnswIndex->finalize(context.getTransaction(), *context.getMemoryManager(), i,
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    return internalCreateHNSWBindFunc(context, input);
}
