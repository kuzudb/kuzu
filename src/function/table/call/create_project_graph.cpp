...
struct CreateProjectGraphBindData : SimpleTableFuncBindData {
    ...
    CreateProjectGraphBindData(std::string graphName, std::vector<TableCatalogEntry*> nodeEntries,
        std::vector<TableCatalogEntry*> relEntries)
        : SimpleTableFuncBindData{0}, graphName{graphName}, nodeEntries{nodeEntries}, relEntries{relEntries} {}
...
static common::offset_t tableFunc(TableFuncInput& input, TableFuncOutput& /*output*/) {
    ...
    if (graphEntrySet.hasGraph(bindData->graphName)) {
        throw RuntimeException(stringFormat("Project graph {} already exists.",  bindData->graphName));
    }
...
static std::vector<catalog::TableCatalogEntry*> getTableEntries(const std::vector<std::string>& tableNames, CatalogEntryType expectedType, main::ClientContext& context) {
    ...
    for (auto& tableName : tableNames) {
        ...
        if (entry->getType() != expectedType) {
            throw BinderException(stringFormat("Expect catalog entry type {} but got {}.", CatalogEntryTypeUtils::toString(expectedType), CatalogEntryTypeUtils::toString(entry->getType())));
        }
...
static std::unique_ptr<TableFuncBindData> bindFunc(main::ClientContext* context,
    ...
    auto nodeEntries = getTableEntries(nodeTableNames, CatalogEntryType::NODE_TABLE_ENTRY, *context);
...
function_set CreateProjectGraphFunction::getFunctionSet() {
    ...
    auto func = std::make_unique<TableFunction>(name, std::vector<LogicalTypeID>{LogicalTypeID::STRING, LogicalTypeID::LIST, LogicalTypeID::LIST});
