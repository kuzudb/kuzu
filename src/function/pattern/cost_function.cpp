...
static std::shared_ptr<Expression> rewriteFunc(const RewriteFunctionBindInput& input) {
    ...
    if (recursiveInfo->bindData->weightOutputExpr == nullptr) {
        throw BinderException(stringFormat("Cost function is not defined for {}", param->toString()));
    }
...
function_set CostFunction::getFunctionSet() {
    ...
    auto function = std::make_unique<RewriteFunction>(name, std::vector<LogicalTypeID>{LogicalTypeID::RECURSIVE_REL}, rewriteFunc);
