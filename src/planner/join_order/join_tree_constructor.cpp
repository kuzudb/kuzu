...
static std::vector<common::idx_t> intersect(std::vector<common::idx_t> left, std::vector<common::idx_t> right) {
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructTreeNode(std::shared_ptr<BoundJoinHintNode> hintNode) {
    // Construct leaf scans.
    if (hintNode->isLeaf()) {
        if (ExpressionUtil::isNodePattern(*hintNode->nodeOrRel)) {
            return constructNodeScan(hintNode->nodeOrRel);
        } else {
            KU_ASSERT(ExpressionUtil::isRelPattern(*hintNode->nodeOrRel) || ExpressionUtil::isRecursiveRelPattern(*hintNode->nodeOrRel));
    ...
    if (hintNode->isBinary()) {
        ...
        auto predicates = Planner::getNewlyMatchedExprs(left.subqueryGraph, right.subqueryGraph, newSubgraph, queryGraphPredicates);
        // First try to construct as index nested loop join.
        auto nestedLoopTreeNode = tryConstructNestedLoopJoin(joinNodes, *left.treeNode, *right.treeNode, predicates);
        ...
        auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::BINARY_JOIN, std::move(extraInfo));
    ...
    for (auto i = 1u; i < hintNode->children.size(); ++i) {
        ...
        if (build.treeNode->type != TreeNodeType::REL_SCAN) {
            throw BinderException(stringFormat("Cannot construct multi-way join because build side is not a relationship table."));
        }
    ...
    extraInfo->predicates = predicates;
    auto treeNode = std::make_shared<JoinTreeNode>(TreeNodeType::MULTIWAY_JOIN, std::move(extraInfo));
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructNodeScan(std::shared_ptr<Expression> expr) {
    ...
    auto predicates = Planner::getNewlyMatchedExprs(emptySubgraph, newSubgraph, queryGraphPredicates);
...
JoinTreeConstructor::IntermediateResult JoinTreeConstructor::constructRelScan(std::shared_ptr<binder::Expression> expr) {
