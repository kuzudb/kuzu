...
public:
    ScanSingleTableProperties(const DataPos& inVectorPos, vector<DataPos> outVectorsPos,
        vector<Column*> columns, unique_ptr<PhysicalOperator> prevOperator, uint32_t id,
    ...
    inline unique_ptr<PhysicalOperator> clone() override {
        return make_unique<ScanSingleTableProperties>(
            inputNodeIDVectorPos, outVectorsPos, columns, children[0]->clone(), id, paramsString);
    }
...
public:
    ...
    inline unique_ptr<PhysicalOperator> clone() override {
        return make_unique<ScanMultiTableProperties>(inputNodeIDVectorPos, outVectorsPos,
            tableIDToColumns, children[0]->clone(), id, paramsString);
    }
