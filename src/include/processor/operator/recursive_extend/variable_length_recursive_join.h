...
public:
    VariableLengthRecursiveJoin(uint8_t lowerBound, uint8_t upperBound,
        storage::NodeTable* nodeTable, storage::RelTable* relTable,
        ...
        : BaseRecursiveJoin{lowerBound, upperBound, nodeTable, relTable, std::move(sharedState),
              std::move(dataInfo), std::move(child), id, paramsString, std::move(recursiveRoot)} {}
    ...
    VariableLengthRecursiveJoin(uint8_t lowerBound, uint8_t upperBound,
        ...
        : BaseRecursiveJoin{lowerBound, upperBound, nodeTable, relTable, std::move(sharedState),
              std::move(datainfo), id, paramsString, std::move(recursiveRoot)} {}

    void initLocalStateInternal(ResultSet* resultSet_, ExecutionContext* context) override;

    inline std::unique_ptr<PhysicalOperator> clone() override {
        return std::make_unique<VariableLengthRecursiveJoin>(lowerBound, upperBound, nodeTable,
            relTable, sharedState, dataInfo->copy(), id, paramsString, recursiveRoot->clone());
    }
};
