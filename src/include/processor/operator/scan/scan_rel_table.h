...
public:
    ScanRelTable(ScanTableInfo info, std::unique_ptr<ScanRelTableInfo> relInfo, std::unique_ptr<PhysicalOperator> child, uint32_t id,
        const std::string& paramsString)
        : ScanRelTable{type_, std::move(info),
              std::move(relInfo), std::move(child), id, paramsString} {}
    ...
    std::unique_ptr<PhysicalOperator> clone() override {
        return std::make_unique<ScanRelTable>(info.copy(), relInfo->copy(),
            children[0]->clone(), id, paramsString);
    }

protected:
    ScanRelTable(PhysicalOperatorType operatorType, ScanTableInfo info, std::unique_ptr<ScanRelTableInfo> relInfo,
        std::unique_ptr<PhysicalOperator> child, uint32_t id, const std::string& paramsString)
        : ScanTable{operatorType, std::move(info), std::move(child), id,
              paramsString},
