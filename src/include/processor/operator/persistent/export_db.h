...
public:
    ExportDB(std::string filePath, common::FileType fileType, common::CSVOption copyToOption,
        uint32_t id, const std::string& paramsString,
        std::vector<std::unique_ptr<PhysicalOperator>> children)
        : PhysicalOperator{PhysicalOperatorType::EXPORT_DATABASE, std::move(children), id,
              paramsString},
          filePath(filePath), fileType(fileType), copyToOption{std::move(copyToOption)} {}
    ...
    inline std::unique_ptr<PhysicalOperator> clone() final {
        return std::make_unique<ExportDB>(
            filePath, fileType, std::move(copyToOption), id, paramsString, std::move(children));
    }
