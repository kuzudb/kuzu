...
/*
 * LocalFrontierSample keeps a small frontier per thread.
 * If enabled, this frontier represents a complete local frontier.
 * Otherwise, complete local frontier is larger than this sample.
 * */
class LocalFrontierSample {
public:
...
private:
    ...
    std::unordered_set<common::offset_t> offsetSet;
    // Local sample disabled meaning frontier is dense and has exceeded SAMPLE_SIZE. This sample
    // no longer represents frontier.
    bool enabled_ = true;
};

/*
 * Sparse frontier keeps a small frontier globally.
