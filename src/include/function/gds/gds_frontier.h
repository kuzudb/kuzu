...
public:
    ...
    void addNode(common::nodeID_t nodeID, iteration_t val);
    void addNodes(const std::vector<common::nodeID_t>& nodeIDs, iteration_t val);
...
public:
    ...
    void setIteration(common::offset_t offset, iteration_t value) {
        KU_ASSERT(curData);
        curData[offset].store(value, std::memory_order_relaxed);
    }

    bool isUnvisited(common::offset_t offset) const {
        KU_ASSERT(curData);
        return curData[offset].load(std::memory_order_relaxed) == UNVISITED;
    }
...
public:
    PathLengthsInitVertexCompute(DenseFrontier& pathLengths, iteration_t val)
...
public:
    explicit SPFrontierPair(std::shared_ptr<DenseFrontier> denseFrontier)
        : denseFrontier {std::move(denseFrontier)}, state{GDSDensityState::SPARSE} {
