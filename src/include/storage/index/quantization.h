...
struct Quantizer {
     ...
     */
    virtual std::unique_ptr<DC<float, T>> get_asym_distance_computer(
        DistanceType type) const = 0;

    /**
     * Get the symmetric distance computer which computes the distance between two quantized
     * vectors. This is faster.
     */
    virtual std::unique_ptr<DC<T, T>> get_sym_distance_computer(
        DistanceType type) const = 0;
...
inline float32x4x2_t decode_neon(const uint8_t* code, size_t i, const float* alpha,
    ...
    // x = alpha * ci + beta
    float32x4_t x_low = vmlaq_f32(vld1q_f32(alpha + i), vld1q_f32(beta + i), ci_vec32_low);
    float32x4_t x_high =
        vmlaq_f32(vld1q_f32(alpha + i + 4), vld1q_f32(beta + i + 4), ci_vec32_high);
    return {x_low, x_high};
}
...
public:
    ...
    std::unique_ptr<SQ8Bit> copy() const {
        return std::make_unique<SQ8Bit>(*this);
    }
