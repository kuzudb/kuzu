...
public:
    ...
    void get(uint64_t idx, transaction::Transaction* transaction, std::span<std::byte> val);

    // Note: This function is to be used only by the WRITE trx.
    void update(transaction::Transaction* transaction, uint64_t idx, std::span<std::byte> val);

    // Note: This function is to be used only by the WRITE trx.
    // The return value is the idx of val in array.
    uint64_t pushBack(transaction::Transaction* transaction, std::span<std::byte> val);

    // Note: Currently, this function doesn't support shrinking the size of the array.
    uint64_t resize(transaction::Transaction* transaction, uint64_t newNumElements,
    ...
    struct WriteIterator {
        ...
        void pushBack(transaction::Transaction* transaction, std::span<std::byte> val);
...
private:
    ...
    std::pair<common::page_idx_t, bool> getAPPageIdxAndAddAPToPIPIfNecessaryForWriteTrxNoLock(
        transaction::Transaction* transaction, common::page_idx_t apIdx);
...
public:
    ...
    // Note: This function is to be used only by the WRITE trx.
    // The return value is the idx of val in array.
    inline uint64_t pushBack(transaction::Transaction* transaction, U val) {
        return diskArray.pushBack(transaction, getSpan(val));
    }

    // Note: This function is to be used only by the WRITE trx.
    inline void update(transaction::Transaction* transaction, uint64_t idx, U val) {
        diskArray.update(transaction, idx, getSpan(val));
    }

    inline U get(uint64_t idx, transaction::Transaction* transaction) {
        U val;
    ...
    // Note: Currently, this function doesn't support shrinking the size of the array.
    inline uint64_t resize(transaction::Transaction* transaction, uint64_t newNumElements) {
        U defaultVal;
    ...
    public:
        ...
        inline WriteIterator& pushBack(transaction::Transaction* transaction, U val) {
