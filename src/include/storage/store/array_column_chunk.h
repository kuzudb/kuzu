...
struct ArrayDataColumnChunk {
    std::unique_ptr<ColumnChunk> dataColumnChunk;
    uint64_t capacity;

    explicit ArrayDataColumnChunk(std::unique_ptr<ColumnChunk> dataChunk)
...
class ArrayColumnChunk : public ColumnChunk {

public:
    ArrayColumnChunk(common::LogicalType dataType, uint64_t capacity, bool enableCompression);

    inline ColumnChunk* getDataColumnChunk() const {
        return arrayDataColumnChunk->dataColumnChunk.get();
    }
    ...
    inline void resizeDataColumnChunk(uint64_t numValues) {
        arrayDataColumnChunk->resizeBuffer(numValues);
    }
...
private:
    ...
    void resetFromOtherChunk(ArrayColumnChunk* other);
    void appendNullList();

protected:
    std::unique_ptr<ArrayDataColumnChunk> arrayDataColumnChunk;
