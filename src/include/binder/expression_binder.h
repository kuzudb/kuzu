...
public:
    ...
    std::shared_ptr<Expression> bindFunctionExpression(
        const parser::ParsedExpression& parsedExpression);
    void bindLambdaExpression(const std::string& functionName, const Expression& lambdaInput, Expression& lambdaExpr);
    std::shared_ptr<Expression> bindLambdaExpression(const parser::ParsedExpression& parsedExpression);
