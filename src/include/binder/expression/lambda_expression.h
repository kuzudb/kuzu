...
public:
    BoundLambdaExpression(std::unique_ptr<parser::ParsedExpression> parsedLambdaExpr, std::string uniqueName)
        : Expression{type_, common::LogicalType::ANY(), uniqueName}, parsedLambdaExpr{std::move(parsedLambdaExpr)} {}
    ...
    std::shared_ptr<Expression> getFunctionExpr() const {
        return functionExpr;
    }
