...
private:
    ...
    std::shared_ptr<planner::LogicalOperator> tryApplySemiMask(std::shared_ptr<binder::Expression> nodeID, std::shared_ptr<planner::LogicalOperator> fromRoot,
        planner::LogicalOperator* toRoot);
    ...
    std::vector<planner::LogicalOperator*> getScanNodeCandidates(
    ...
    std::vector<planner::LogicalOperator*> getRecursiveJoinCandidates(
        const binder::Expression& nodeID, planner::LogicalOperator* root);
    std::vector<planner::LogicalOperator*> getGDSCallCandidates(
        const binder::Expression& nodeID, planner::LogicalOperator* root);
};
