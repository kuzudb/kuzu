...
public:
    LogicalGDSCall(binder::BoundGDSCallInfo info, std::unique_ptr<OPPrintInfo> printInfo)
        : LogicalOperator{operatorType_, std::move(printInfo)}, info{std::move(info)} {}
    LogicalGDSCall(binder::BoundGDSCallInfo info,
        std::shared_ptr<LogicalOperator> nodePredicateRoot, std::unique_ptr<OPPrintInfo> printInfo)
        : LogicalOperator{operatorType_, std::move(printInfo)}, info{std::move(info)},
    ...
    std::unique_ptr<LogicalOperator> copy() override {
        return std::make_unique<LogicalGDSCall>(info.copy(), nodePredicateRoot, printInfo->copy());
    }
