    ...
    for (auto i = 0u; i < height; i++) {
        for (auto j = 0u; j < opProfileBoxes[rowIdx].size(); j++) {
            ...
            if (opProfileBox &&
                ...
                if (hasOpProfileBox(rowIdx + 1, j) && i >= halfWayPoint) {
                    ...
                    if (i == halfWayPoint) {
                        ...
                        if (hasOpProfileBox(rowIdx + 1, j + 1) && !hasOpProfileBox(rowIdx, j + 1)) {
                            ...
                            if (hasOpProfileBox(rowIdx + 1, j-1) && !hasOpProfileBox(rowIdx, j) && !hasOpProfileBox(rowIdx, j+1)) {
                                oss << "┬" 
                                    << genHorizLine(opProfileBoxWidth/2);
                            }
                            else{
                                oss << "┐"
                ...
                } else if (((hasOpProfileBox(rowIdx + 1, j + 1) && !hasOpProfileBox(rowIdx, j + 1)) || (hasOpProfileBox(rowIdx + 1, j - 1) && !hasOpProfileBox(rowIdx, j - 1)) || (hasOpProfileBox(rowIdx + 1, j - 2) && !hasOpProfileBox(rowIdx, j - 2))) &&
                           i == halfWayPoint) {
