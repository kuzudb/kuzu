...
std::unique_ptr<PreparedStatement> ClientContext::prepare(std::string_view query) {
    return prepareInternal(query, false /*enumerateAllPlans*/, "" /*joinOrder*/);
}
...
std::unique_ptr<PreparedStatement> ClientContext::prepareInternal(std::string_view query,
    ...
    return prepareNoLock(parsedStatements[0], enumerateAllPlans, joinOrder);
}
...
std::unique_ptr<PreparedStatement> ClientContext::prepareNoLock(
    ...
    try {
        ...
        if (encodedJoin.empty()) {
            ...
            preparedStatement->logicalPlans.push_back(findMatchingEncodedPlan(plans, encodedJoin));
        }
...
std::unique_ptr<planner::LogicalPlan> ClientContext::findMatchingEncodedPlan(
    std::vector<std::unique_ptr<planner::LogicalPlan>>& plans, std::string_view encodedJoin) {
...
std::vector<std::shared_ptr<Statement>> ClientContext::parseQuery(std::string_view query) {
    ...
    KU_ASSERT(transactionContext->hasActiveTransaction());
    return Parser::parseQuery(query, this);
}
...
std::unique_ptr<QueryResult> ClientContext::executeWithParams(PreparedStatement* preparedStatement,
    ...
    auto rebindPreparedStatement = prepareNoLock(preparedStatement->parsedStatement,
        false /*enumerateAllPlans*/, "" /*joinOrder*/, preparedStatement->parameterMap);
