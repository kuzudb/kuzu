...
std::unique_ptr<PhysicalOperator> PlanMapper::mapUseDatabase(
    ...
    auto printInfo = std::make_unique<UseDatabasePrintInfo>(useDatabase->getExpressionsForPrinting());
...
std::unique_ptr<PhysicalOperator> PlanMapper::mapDetachDatabase(
    ...
    auto printInfo = std::make_unique<DetachDatabasePrintInfo>(detachDatabase->getDBName());
