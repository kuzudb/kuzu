...
std::unique_ptr<PhysicalOperator> PlanMapper::mapRecursiveExtend(LogicalOperator* logicalOperator) {
    ...
    dataInfo.recursiveSrcNodeIDPos = getDataPos(*recursiveInfo->node->getInternalID(), *recursivePlanSchema);
    dataInfo.recursiveNodePredicateExecFlagPos = getDataPos(*recursiveInfo->nodePredicateExecFlag, *recursivePlanSchema);
