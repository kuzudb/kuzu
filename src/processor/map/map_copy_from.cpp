...
std::unique_ptr<PhysicalOperator> PlanMapper::mapCopyNodeFrom(LogicalOperator* logicalOperator) {
    ...
    auto printInfo = std::make_unique<NodeBatchInsertPrintInfo>(
        copyFrom.getInfo()->tableEntry->getName(), copyFrom.getInfo()->source->type);
...
std::unique_ptr<PhysicalOperator> PlanMapper::createCopyRel(
    ...
    auto printInfo = std::make_unique<RelBatchInsertPrintInfo>(
        copyFrom.getInfo()->tableEntry->getName(), copyFrom.getInfo()->source->type);
