...
uint64_t CopyNode::waitForOneNodeGroup() {
    uint64_t numOfReq = INT64_MAX;
    bool needToWait = false;
    for (auto& groupInfo: nodeGroupInfo) {
        if (groupInfo->receivedReq != groupInfo->totalReq) {
            needToWait = true;
            if (numOfReq > groupInfo->totalReq - groupInfo->receivedReq) {
                numOfReq = groupInfo->totalReq - groupInfo->receivedReq;
            }
        }
    }
    return needToWait ? numOfReq : 0;
}

void CopyNode::executeInternal(ExecutionContext* context) {
    auto ret = io_uring_queue_init(200, &ring, 0);
    if (ret < 0) {
        throw Exception("Queue init failed.");
    }

    int fds[1];
    fds[0] = getColumnDataFd();
    ret = io_uring_register_files(&ring, fds, 1);
    if (ret < 0) {
        throw Exception("Registration of file descriptor failed.");
    }
    ...
    uint64_t reqToWait =  waitForOneNodeGroup();
    while(reqToWait) {
        ret = io_uring_wait_cqe_nr(&ring, &cqe, reqToWait);
        if (ret < 0) {
            throw Exception(strerror(-ret));
        }

        for (auto i = 0u; i < reqToWait; i++) {
            ret = io_uring_peek_cqe(&ring, &cqe);
            if (ret < 0) {
                throw Exception("error response when peek");
            }
...
void CopyNode::writeNodeGroup(node_group_idx_t nodeGroupIdx,
    ...
    struct io_uring_cqe *cqe;
    uint64_t reqToWait = waitForOneNodeGroup();
    auto ret = io_uring_wait_cqe_nr(&ring, &cqe, reqToWait);
    if (ret < 0) {
