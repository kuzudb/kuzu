...
std::unique_ptr<PhysicalOperator> SetNodeProperty::clone() {
    return std::make_unique<SetNodeProperty>(NodeSetExecutor::copy(executors), children[0]->clone(), id, paramsString);
}
...
std::unique_ptr<PhysicalOperator> SetRelProperty::clone() {
    return std::make_unique<SetRelProperty>(RelSetExecutor::copy(executors), children[0]->clone(), id, paramsString);
}
