...
HashAggregateSharedState::HashAggregateSharedState(main::ClientContext* context,
    ...
    // TODO(bmwinger): can probably skip this for the hash column, which should never contain nulls
    ...
    for (auto& partition : globalPartitions) {
        ...
        // TODO(bmwinger): Each partition is the same, so we should be able to just do this once and
        // copy it to the other partitions
        for (size_t functionIdx = 0; functionIdx < aggregateFunctions.size(); functionIdx++) {
            ...
            if (function.isFunctionDistinct()) {
                ...
                for (size_t i = 0;
                     i < this->aggInfo.flatKeysPos.size() + this->aggInfo.unFlatKeysPos.size();
                     i++) {
                    distinctTableSchema.appendColumn(
                        this->aggInfo.tableSchema.getColumn(i)->copy());
                ...
                distinctTableSchema.setMayContainsNullsToTrue(
                    distinctTableSchema.getNumColumns() - 1);
                // Hash column
                distinctTableSchema.appendColumn(
