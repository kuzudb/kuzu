...
public:
    UpdateNode(uint64_t dataChunkPos, vector<uint32_t> propertyKeyVectorPos, label_t nodeLabel,
        vector<BaseColumn*> nodePropertyColumns, uint64_t numNodes,
        unique_ptr<PhysicalOperator> prevOperator, ExecutionContext& context, uint32_t id)
        : CRUDNode{UPDATE_NODE, dataChunkPos, move(propertyKeyVectorPos), nodeLabel,
              move(nodePropertyColumns), numNodes, move(prevOperator), context, id} {};

    void getNextTuples() override;

    unique_ptr<PhysicalOperator> clone() override {
        return make_unique<UpdateNode>(dataChunkPos, propertyKeyVectorPos, nodeLabel,
