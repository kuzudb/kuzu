...
FactorizedTable::FactorizedTable(MemoryManager* memoryManager, FactorizedTableSchema tableSchema)
    ...
    if (!this->tableSchema.isEmpty()) {
        ...
        if (numBytesPerTuple > BufferPoolConstants::TEMP_PAGE_SIZE) {
        ...
        flatTupleBlockSize = BufferPoolConstants::TEMP_PAGE_SIZE;
...
uint64_t getDataBlockSize(uint32_t numBytes) {
    if (numBytes < BufferPoolConstants::TEMP_PAGE_SIZE) {
        return BufferPoolConstants::TEMP_PAGE_SIZE;
    }
    return numBytes + 1;
}
