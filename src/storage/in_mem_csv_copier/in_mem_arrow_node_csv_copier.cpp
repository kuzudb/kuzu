...
arrow::Status InMemArrowNodeCSVCopier::initializeArrow(const string& arrowFilePath) {
    ARROW_ASSIGN_OR_RAISE(
            infile,
            arrow::io::ReadableFile::Open(arrowFilePath, arrow::default_memory_pool()));
...
template<typename T>
void InMemArrowNodeCSVCopier::populateColumnsAndCountUnstrPropertyListSizesTask(uint64_t IDColumnIdx,
...
void InMemArrowNodeCSVCopier::calculateArrowNumBlocks(const std::string &arrowFilePath, std::string tableName) {
