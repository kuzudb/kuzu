...
ListChunkData::ListChunkData(LogicalType dataType, uint64_t capacity, bool enableCompression,
    ResidencyState type)
    : ColumnChunkData{std::move(dataType), capacity, enableCompression, type,
          true /*hasNullData*/} {
...
void ListChunkData::append(ColumnChunkData* other, offset_t startPosInOtherChunk,
    ...
    offsetColumnChunk->getNullData()->append(other->getNullData(), startPosInOtherChunk,
        numValuesToAppend);
    sizeColumnChunk->getNullData()->append(other->getNullData(), startPosInOtherChunk,
        numValuesToAppend);
...
void ListChunkData::append(ValueVector* vector, const SelectionVector& selVector) {
    ...
    for (auto i = 0u; i < selVector.getSelSize(); i++) {
        ...
        sizeColumnChunk->getNullData()->setNull(appendBaseOffset + i, vector->isNull(pos));
        ...
        offsetColumnChunk->getNullData()->setNull(appendBaseOffset + i, vector->isNull(pos));
    }
...
void ListChunkData::appendNullList() {
    ...
    sizeColumnChunk->getNullData()->setNull(appendPosition, true);
    setOffsetChunkValue(nextListOffsetInChunk, appendPosition);
    offsetColumnChunk->getNullData()->setNull(appendPosition, true);
    getNullData()->setNull(appendPosition, true);
}
...
void ListChunkData::write(ColumnChunkData* chunk, ColumnChunkData* dstOffsets, RelMultiplicity) {
    ...
    for (auto i = 0u; i < dstOffsets->getNumValues(); i++) {
        ...
        offsetColumnChunk->getNullData()->setNull(posInChunk, otherListChunk.nullData->isNull(i));
        sizeColumnChunk->setValue<list_size_t>(appendSize, posInChunk);
        sizeColumnChunk->getNullData()->setNull(posInChunk, otherListChunk.nullData->isNull(i));
    }
    KU_ASSERT(sanityCheck());
}
...
void ListChunkData::write(const ValueVector* vector, offset_t offsetInVector,
    ...
    offsetColumnChunk->getNullData()->setNull(offsetInChunk, isNull);
    sizeColumnChunk->getNullData()->setNull(offsetInChunk, isNull);
    if (!isNull) {
...
void ListChunkData::write(ColumnChunkData* srcChunk, offset_t srcOffsetInChunk,
    ...
    for (auto i = 0u; i < numValuesToCopy; i++) {
        ...
        offsetColumnChunk->getNullData()->setNull(dstOffsetInChunk + i,
            srcListChunk.nullData->isNull(srcOffsetInChunk + i));
        sizeColumnChunk->getNullData()->setNull(dstOffsetInChunk + i,
            srcListChunk.nullData->isNull(srcOffsetInChunk + i));
    }
    dataColumnChunk->resize(offsetInDataChunkToAppend);
    for (auto i = 0u; i < numValuesToCopy; i++) {
...
void ListChunkData::finalize() {
    ...
    for (auto i = 0u; i < numValues; i++) {
        if (nullData->isNull(i)) {
            ...
            newListChunk.sizeColumnChunk->getNullData()->setNull(currentIndex, false);
            newListChunk.sizeColumnChunk->setValue<list_size_t>(listSize, currentIndex);
            newListChunk.offsetColumnChunk->getNullData()->setNull(currentIndex, false);
