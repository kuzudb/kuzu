...
public:
    ...
    void scan(
        Transaction* transaction, node_group_idx_t nodeGroupIdx, ColumnChunk* columnChunk) final {
...
void Column::prepareCommitForChunk(Transaction* transaction, node_group_idx_t nodeGroupIdx,
    ...
    const offset_to_row_idx_t& updateInfo, const std::unordered_set<common::offset_t>& deleteInfo) {
    ...
    if (nodeGroupIdx >= numNodeGroups) {
        ...
        commitLocalChunkOutOfPlace(transaction, localChunk, insertInfo, updateInfo, deleteInfo,
            nodeGroupIdx, true /* isNewChunk */);
    } else {
...
void Column::commitLocalChunkOutOfPlace(Transaction* transaction, LocalVectorCollection* localChunk,
    const offset_to_row_idx_t& insertInfo, const offset_to_row_idx_t& updateInfo,
