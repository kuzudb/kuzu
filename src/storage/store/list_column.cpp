...
void ListColumn::scanInternal(Transaction* transaction, const ChunkState& state, idx_t vectorIdx,
    ...
    void ListColumn::fastLookup(TransactionType txnType, FastLookupRequest *request,
    ...
    for (auto i = 0u; i < request->size; i++) {
        auto nodeOffset = request->nodeOffsets[i];
        auto& readState = *request->states[i];
        auto nodeGroupIdx = StorageUtils::getNodeGroupIdx(nodeOffset);
        KU_ASSERT(readState.nodeGroupIdx == nodeGroupIdx);
        auto nodeOffsetInGroup = (nodeOffset - StorageUtils::getStartOffsetOfNodeGroup(nodeGroupIdx)) * size;
...
//        auto listStartOffset = listEndOffset - size;
        request->states[i] = &readState.childrenStates[DATA_COLUMN_CHILD_READ_STATE_IDX];
        request->startOffsetsInGroup[i] = nodeOffsetInGroup;
        request->endOffsetsInGroup[i] = nodeOffsetInGroup + size;
    }
    dataColumn->fastScan(txnType, request, computeFunc);
}
