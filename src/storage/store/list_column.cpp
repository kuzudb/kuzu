...
void ListColumn::initChunkState(Transaction* transaction, node_group_idx_t nodeGroupIdx,
    ...
    sizeColumn->initChunkState(transaction, nodeGroupIdx,
        readState.childrenStates[ListColumnChunk::SIZE_COLUMN_CHILD_READ_STATE_IDX]);
    dataColumn->initChunkState(transaction, nodeGroupIdx,
        readState.childrenStates[ListColumnChunk::DATA_COLUMN_CHILD_READ_STATE_IDX]);
}
...
void ListColumn::scan(Transaction* transaction, const ChunkState& state,
    ...
    if (isOffsetSortedAscending) {
        dataColumn->scan(transaction,
            state.childrenStates[ListColumnChunk::DATA_COLUMN_CHILD_READ_STATE_IDX],
...
void ListColumn::append(ColumnChunk* columnChunk, ChunkState& state) {
    ...
    sizeColumn->append(sizeColumnChunk,
        state.childrenStates[ListColumnChunk::SIZE_COLUMN_CHILD_READ_STATE_IDX]);
...
void ListColumn::setMetadataFromChunk(node_group_idx_t nodeGroupIdx, const ColumnChunk& chunk) {
...
void ListColumn::setMetadataToChunk(node_group_idx_t nodeGroupIdx, ColumnChunk& chunk) const {
    Column::setMetadataToChunk(nodeGroupIdx, chunk);
    auto& listChunk = chunk.cast<ListColumnChunk>();
...
void ListColumn::prepareCommitForExistingChunk(Transaction* transaction, ChunkState& state,
    ...
    auto dataColumnSize =
        state.childrenStates[ListColumnChunk::DATA_COLUMN_CHILD_READ_STATE_IDX].metadata.numValues;
    ...
    if (!dataColumnCanCommitInPlace) {
        ...
        sizeColumn->prepareCommitForExistingChunk(transaction,
            state.childrenStates[ListColumnChunk::SIZE_COLUMN_CHILD_READ_STATE_IDX], dstOffsets,
