...
void ColumnChunkData::serialize(Serializer& serializer) const {
    ...
    auto serializedMetadata = metadata.serializeToBytes(dataType.getPhysicalType());
    serializer.write(reinterpret_cast<uint8_t*>(serializedMetadata.data()),
...
std::unique_ptr<ColumnChunkData> ColumnChunkData::deserialize(Deserializer& deSer) {
    ...
    std::vector<std::byte> metadataBytes(
        ColumnChunkMetadata::serializedSizeBytes(dataType.getPhysicalType()));
    deSer.read(reinterpret_cast<uint8_t*>(metadataBytes.data()), metadataBytes.size());
    metadata.deserializeFromBytes(metadataBytes, dataType.getPhysicalType());
...
std::unique_ptr<NullChunkData> NullChunkData::deserialize(Deserializer& deSer) {
    ...
    std::vector<std::byte> metadataBytes(
        ColumnChunkMetadata::serializedSizeBytes(PhysicalTypeID::BOOL));
    deSer.read(reinterpret_cast<uint8_t*>(metadataBytes.data()), metadataBytes.size());
    metadata.deserializeFromBytes(metadataBytes, PhysicalTypeID::BOOL);
