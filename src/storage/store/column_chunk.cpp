...
ColumnChunkMetadata fixedSizedFlushBuffer(const uint8_t* buffer, uint64_t bufferSize,
    ...
    if (loop) {
        dataFH->getFileInfo()->writeFileAsync(
            buffer, bufferSize, startPageIdx * BufferPoolConstants::PAGE_4KB_SIZE, loop, info);
    } else {
...
public:
    ...
    ColumnChunkMetadata operator()(const uint8_t* buffer, uint64_t /*bufferSize*/,
        ...
        while (valuesRemaining > 0) {
            ...
            if (loop) {
                dataFH->getFileInfo()->writeFileAsync(compressedBuffer.get(),
...
ColumnChunkMetadata ColumnChunk::flushBuffer(
    ...
    if (!metadata.compMeta.isConstant()) {
        return flushBufferFunction(
            buffer.get(), bufferSize, dataFH, startPageIdx, metadata, nullptr, nullptr);
    }
    return metadata;
}
...
ColumnChunkMetadata ColumnChunk::flushBufferAsync(BMFileHandle* dataFH, page_idx_t startPageIdx,
    ...
    if (!metadata.compMeta.isConstant()) {
        return flushBufferFunction(
            buffer.get(), bufferSize, dataFH, startPageIdx, metadata, loop, info);
    }
    return metadata;
}
