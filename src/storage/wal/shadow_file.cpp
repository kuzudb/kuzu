...
void ShadowFile::replayShadowPageRecords(ClientContext& context) const {
    ...
    const auto pageBuffer = std::make_unique<uint8_t[]>(BufferPoolConstants::PAGE_SIZE);
    ...
    for (const auto& record : shadowPageRecords) {
        ...
        fileInfoOfDBFile->writeFile(pageBuffer.get(), BufferPoolConstants::PAGE_SIZE,
            record.originalPageIdx * BufferPoolConstants::PAGE_SIZE);
...
void ShadowFile::flushAll() const {
    ...
    const auto headerBuffer = std::make_unique<uint8_t[]>(BufferPoolConstants::PAGE_SIZE);
    ...
    writer->setFileOffset(shadowingFH->getNumPages() * BufferPoolConstants::PAGE_SIZE);
