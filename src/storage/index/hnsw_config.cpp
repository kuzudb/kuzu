...
HNSWIndexConfig::HNSWIndexConfig(const function::optional_params_t& optionalParams) {
    for (auto& [name, value] : optionalParams) {
        ...
        if (M::NAME == lowerCaseName) {
            value.validateType(M::TYPE);
            connectivityInUpperLayer = value.getValue<int64_t>();
            M::validate(connectivityInUpperLayer);
        } else if (M0::NAME == lowerCaseName) {
            value.validateType(M0::TYPE);
            connectivityInLowerLayer = value.getValue<int64_t>();
            M0::validate(connectivityInLowerLayer);
...
void HNSWIndexConfig::serialize(common::Serializer& ser) const {
    ...
    ser.serializeValue(connectivityInUpperLayer);
    ser.writeDebuggingInfo("degreeInLowerLayer");
    ser.serializeValue(connectivityInLowerLayer);
...
HNSWIndexConfig HNSWIndexConfig::deserialize(common::Deserializer& deSer) {
    ...
    deSer.deserializeValue(config.connectivityInUpperLayer);
    deSer.validateDebuggingInfo(debugginInfo, "degreeInLowerLayer");
    deSer.deserializeValue(config.connectivityInLowerLayer);
