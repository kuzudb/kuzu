...
template<typename T>
HashIndex<T>::HashIndex(const DBFileIDAndName& dbFileIDAndName, const LogicalType& keyDataType,
    ...
    if (keyDataType.getLogicalTypeID() == LogicalTypeID::STRING) {
        diskOverflowFile = std::make_unique<DiskOverflowFile>(dbFileIDAndName, &bm, wal);
    }
}
