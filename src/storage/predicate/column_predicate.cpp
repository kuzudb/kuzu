...
static bool isPropertyConstantPair(const Expression& left, const Expression& right) {
    return left.expressionType == ExpressionType::PROPERTY && right.expressionType == ExpressionType::LITERAL;
}

static std::unique_ptr<ColumnPredicate> tryConvertToConstColumnPredicate(const Expression& property, const Expression& predicate) {
    if (isPropertyConstantPair(*predicate.getChild(0), *predicate.getChild(1))) {
        ...
        auto expressionType = ExpressionTypeUtil::reverseComparisonDirection(predicate.expressionType);
...
std::unique_ptr<ColumnPredicate> ColumnPredicateUtil::tryConvert(const Expression& property, const Expression& predicate) {
