...
void UnstructuredPropertyLists::readPropertiesForPosition(ValueVector* nodeIDVector, uint32_t pos,
    ...
    PageCursor cursor{info.cursor.pageIdx, info.cursor.posInPage};
...
unique_ptr<map<uint32_t, Literal>> UnstructuredPropertyLists::readUnstructuredPropertiesOfNode(
    ...
    PageCursor byteCursor{info.cursor.pageIdx, info.cursor.posInPage};
    auto propertyKeyDataType = UnstructuredPropertyKeyDataType{UINT32_MAX, ANY};
    auto numBytesRead = 0u;
    while (numBytesRead < info.listLen) {
...
void UnstructuredPropertyLists::readPropertyKeyAndDatatype(uint8_t* propertyKeyDataType,
    PageCursor& cursor, const function<uint32_t(uint32_t)>& logicalToPhysicalPageMapper) {
    auto totalNumBytesRead = 0u;
    auto bytesInCurrentPage = DEFAULT_PAGE_SIZE - cursor.posInPage;
...
void UnstructuredPropertyLists::skipPropertyValue(uint64_t dataTypeSize, PageCursor& cursor) {
    auto bytesToReadInCurrentPage = min(dataTypeSize, DEFAULT_PAGE_SIZE - cursor.posInPage);
    cursor.posInPage += bytesToReadInCurrentPage;
    if (dataTypeSize > bytesToReadInCurrentPage) {
        cursor.pageIdx++;
        cursor.posInPage = dataTypeSize - bytesToReadInCurrentPage;
    }
}

void UnstructuredPropertyLists::readFromAPage(uint8_t* value, uint64_t bytesToRead,
    PageCursor& cursor, const std::function<uint32_t(uint32_t)>& logicalToPhysicalPageMapper) {
    ...
    memcpy(value, frame + cursor.posInPage, bytesToRead);
    bufferManager.unpin(fileHandle, physicalPageIdx);
    cursor.posInPage += bytesToRead;
}
