...
std::unique_ptr<TableCatalogEntry> RelGroupCatalogEntry::alter(transaction_t timestamp,
    const binder::BoundAlterInfo& alterInfo) const {
    std::unique_ptr<TableCatalogEntry> newEntry;
...
const RelTableCatalogInfo& RelGroupCatalogEntry::getSingleRelTableInfo() const {
    KU_ASSERT(relTableInfos.size() == 1);
    return relTableInfos[0];
}

const RelTableCatalogInfo* RelGroupCatalogEntry::getRelTableInfo(table_id_t srcTableID,
...
static std::string getFromToStr(const NodeTableIDPair& pair, Catalog* catalog,
    ...
    auto srcTableName =
        catalog->getTableCatalogEntry(transaction, pair.srcTableID)->getName();
    auto dstTableName =
        catalog->getTableCatalogEntry(transaction, pair.dstTableID)->getName();
...
std::string RelGroupCatalogEntry::toCypher(const ToCypherInfo& info) const {
    ...
    ss << ", " << propertyCollection.toCypher() << RelMultiplicityUtils::toString(srcMultiplicity) << "_" << RelMultiplicityUtils::toString(dstMultiplicity) << ");"; return ss.str();
}
