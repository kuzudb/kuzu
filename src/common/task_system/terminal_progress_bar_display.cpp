...
void TerminalProgressBarDisplay::updateProgress(std::string /*id*/, double newPipelineProgress,
    uint32_t newNumPipelinesFinished) {
    uint32_t pipelineProgress = (uint32_t)(newPipelineProgress * 100.0);
    uint32_t oldPipelineProgress = (uint32_t)(pipelineProgress * 100.0);
    if (pipelineProgress > oldPipelineProgress || newNumPipelinesFinished > numPipelinesFinished) {
...
void TerminalProgressBarDisplay::finishProgress(std::string /*id*/) {
    if (printing) {
