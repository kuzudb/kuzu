...
public:
    ...
    // chain. There are no child contexts stored here and often the parent context is left dangling
    // when it's owning ATNState is released. In order to avoid having this context released as well
    // (leaving all other contexts which got this one as parent with a null reference) we use a
