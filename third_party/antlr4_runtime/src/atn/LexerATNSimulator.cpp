...
dfa::DFAState* LexerATNSimulator::computeTargetState(
    CharStream* input, dfa::DFAState* s, size_t t) {
...
size_t LexerATNSimulator::failOrAccept(CharStream* input, ATNConfigSet* reach, size_t t) {
    if (_prevAccept.dfaState != nullptr) {
        ...
        accept(input, lexerActionExecutor, _startIndex, _prevAccept.index, _prevAccept.line,
            _prevAccept.charPos);
...
void LexerATNSimulator::getReachableConfigSet(
    CharStream* input, ATNConfigSet* closure_, ATNConfigSet* reach, size_t t) {
        ...
        for (size_t ti = 0; ti < n; ti++) { // for each transition
            ...
            if (target != nullptr) {
                ...
                Ref<LexerATNConfig> config = std::make_shared<LexerATNConfig>(
                    std::static_pointer_cast<LexerATNConfig>(c), target, lexerActionExecutor);
...
void LexerATNSimulator::accept(CharStream* input,
    const Ref<LexerActionExecutor>& lexerActionExecutor, size_t /*startIndex*/, size_t index,
...
bool LexerATNSimulator::closure(CharStream* input, const Ref<LexerATNConfig>& config,
    ATNConfigSet* configs, bool currentAltReachedAcceptState, bool speculative,
        ...
        if (config->context != nullptr && !config->context->isEmpty()) {
            for (size_t i = 0; i < config->context->size(); i++) {
                if (config->context->getReturnState(i) != PredictionContext::EMPTY_RETURN_STATE) {
                    ...
                    Ref<LexerATNConfig> c =
                        std::make_shared<LexerATNConfig>(config, returnState, newContext.lock());
...
Ref<LexerATNConfig> LexerATNSimulator::getEpsilonTarget(CharStream* input,
    const Ref<LexerATNConfig>& config, Transition* t, ATNConfigSet* configs, bool speculative,
    bool treatEofAsEpsilon) {
...
bool LexerATNSimulator::evaluatePredicate(
    CharStream* input, size_t ruleIndex, size_t predIndex, bool speculative) {
