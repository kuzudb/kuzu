...
using namespace kuzu_parquet;
using namespace kuzu_parquet::format;
using namespace kuzu_apache::thrift;
using namespace kuzu_apache::thrift::protocol;
using namespace kuzu_apache::thrift::transport;
...
void ParquetFile::initialize(string filename) {
    ...
    if (file_meta_data.schema[0].num_children != file_meta_data.schema.size() - 1) {
...
public:
    ...
    inline int GetBatch(T* values, int batch_size) {
        ...
        while (values_read < batch_size) {
            if (repeat_count_ > 0) {
                int repeat_batch =
                    std::min(batch_size - values_read, static_cast<uint32_t>(repeat_count_));
                std::fill(values + values_read, values + values_read + repeat_batch,
                    static_cast<T>(current_value_));
                repeat_count_ -= repeat_batch;
                values_read += repeat_batch;
            } else if (literal_count_ > 0) {
                uint32_t literal_batch =
                    std::min(batch_size - values_read, static_cast<uint32_t>(literal_count_));
    ...
    template<typename T>
    inline int GetBatchSpaced(
        uint32_t batch_size, uint32_t null_count, const uint8_t* defined, T* out) {
...
const uint32_t RleBpDecoder::BITPACK_MASKS[] = {0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047,
    4095, 8191, 16383, 32767, 65535, 131071, 262143, 524287, 1048575, 2097151, 4194303, 8388607,
...
public:
    ...
    void scan_dict_page(ResultColumn& result_col) {
        if (page_header.__isset.data_page_header || !page_header.__isset.dictionary_page_header) {
        ...
        case Type::BYTE_ARRAY:
            ...
            {
                // never going to have more string data than this uncompressed_page_size (lengths
                // use bytes)
                auto string_heap_chunk =
                    std::unique_ptr<char[]>(new char[page_header.uncompressed_page_size]);
                result_col.string_heap_chunks.push_back(move(string_heap_chunk));
                auto str_ptr =
                    result_col.string_heap_chunks[result_col.string_heap_chunks.size() - 1].get();
        ...
        default:
            throw runtime_error(
                "Unsupported type for dictionary: " + type_to_string(result_col.col->type));
        }
    }
    ...
    void fill_values_plain(ResultColumn& result_col) {
        ...
        for (int32_t val_offset = 0; val_offset < page_header.data_page_header.num_values;
             val_offset++) {
    ...
    void scan_data_page_plain(ResultColumn& result_col) {
        ...
        default:
            throw runtime_error(
                "Unsupported type page_plain " + type_to_string(result_col.col->type));
        }
    }
    ...
    // here we look back into the dicts and emit the values we find if the value is defined,
    // otherwise NULL
    void scan_data_page_dict(ResultColumn& result_col) {
        ...
        if (enc_length > 0) {
            ...
            if (null_count > 0) {
                dec.GetBatchSpaced<uint32_t>(num_values, null_count, defined_ptr, offsets.get());
            } else {
        ...
        default:
            throw runtime_error(
                "Unsupported type page_dict " + type_to_string(result_col.col->type));
        }
    }
...
void ParquetFile::scan_column(ScanState& state, ResultColumn& result_col) {
    ...
    while (bytes_to_read > 0) {
        auto page_header_len =
            bytes_to_read; // the header is clearly not that long but we have no idea

        // this is the only other place where we actually unpack a thrift object
        cs.page_header = PageHeader();
        thrift_unpack((const uint8_t*)chunk_buf.ptr, (uint32_t*)&page_header_len, &cs.page_header);
        //
        ...
        case CompressionCodec::SNAPPY: {
            ...
            kuzu_snappy::GetUncompressedLength(
                chunk_buf.ptr, cs.page_header.compressed_page_size, &decompressed_size);
            decompressed_buf.resize(decompressed_size + 1);

            auto res = kuzu_snappy::RawUncompress(
                chunk_buf.ptr, cs.page_header.compressed_page_size, decompressed_buf.ptr);
            if (!res) {
