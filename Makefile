...
extension-test: extension-test-build
    ...
    ctest --test-dir build/$(call get-build-path,RelWithDebInfo)/{test/runner,extension} --output-on-failure -j ${TEST_JOBS} --exclude-regex "${EXTENSION_TEST_EXCLUDE_FILTER}" && \
