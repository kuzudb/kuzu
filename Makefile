...
test:
	...
	E2E_TEST_FILES_DIRECTORY=test/test_files ctest --test-dir build/relwithdebinfo/test --output-on-failure -j ${TEST_JOBS} --test-args "TESTESTSESTEST"
...
extension-test: extension-test-build
	E2E_TEST_FILES_DIRECTORY=extension ctest --test-dir build/relwithdebinfo/test --output-on-failure -j ${TEST_JOBS} --exclude-regex "${EXTENSION_TEST_EXCLUDE_FILTER}" -R e2e_test --test-args "TESTESTSESTEST"
