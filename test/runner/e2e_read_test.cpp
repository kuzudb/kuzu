...
class LongStringPKTest : public DBTest {
    string getInputCSVDir() override { return TestHelper::appendKuzuRootPath("dataset/long-string-pk-tests/"); }
};
...
TEST_F(TinySnbReadTest, Projection) {
    ...
    runTest(TestHelper::appendKuzuRootPath("test/test_files/tinySNB/projection/multi_query_part.test"));
}
...
TEST_F(TinySnbReadTest, OptionalMatch) {
    runTest(TestHelper::appendKuzuRootPath("test/test_files/tinySNB/optional_match/optional_match.test"));
}

TEST_F(TinySnbReadTest, OrderBy) {
    auto queryConfigs = TestHelper::parseTestFile(
        TestHelper::appendKuzuRootPath("test/test_files/tinySNB/order_by/order_by.test"), true /* checkOutputOrder */);
...
TEST_F(TinySnbReadTest, VarLengthExtendTests) {
    runTest(TestHelper::appendKuzuRootPath("test/test_files/tinySNB/var_length_extend/var_length_adj_list_extend.test"));
    runTest(TestHelper::appendKuzuRootPath("test/test_files/tinySNB/var_length_extend/var_length_column_extend.test"));
}
