...
public:
    string getInputCSVDir() override {
        return TestHelper::appendKuzuRootPath("dataset/read-list-tests/large-list-sub-query-tests/");
    }
};
...
TEST_F(EndToEndReadLists2BytesPerEdgeTest, PropLists4BytesPerEdgeTest) {
    runTest(TestHelper::appendKuzuRootPath("test/test_files/read_list/small-large-property-list-reading.test"));
}

TEST_F(EndToEndReadLists2BytesPerEdgeTest, VarLengthExtendLargeAdjListTest) {
    runTest(TestHelper::appendKuzuRootPath("test/test_files/read_list/var_length_large_adj_list_extend.test"));
}
