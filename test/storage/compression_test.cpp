...
TEST(CompressionTests, IntegerBitpackingMetadataInvalidPhysicalType) {
    ...
    EXPECT_THROW(metadata.numValues(BufferPoolConstants::PAGE_SIZE, LogicalType::STRING()),
        StorageException);
...
TEST(CompressionTests, FloatCompressionMetadataInvalidPhysicalType) {
    ...
    EXPECT_THROW(metadata.numValues(BufferPoolConstants::PAGE_SIZE, LogicalType::UINT64()),
        StorageException);
