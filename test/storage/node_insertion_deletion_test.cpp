...
TEST_F(NodeInsertionDeletionTests, InsertManyNodesTest) {
    ...
    for (int64_t i = 0; i < (int64_t)BufferPoolConstants::PAGE_SIZE; i++) {
        auto result =
    ...
    ASSERT_EQ(tuple->getValue(0)->getValue<int64_t>(), BufferPoolConstants::PAGE_SIZE);
    ...
    ASSERT_EQ(i, BufferPoolConstants::PAGE_SIZE);
}
