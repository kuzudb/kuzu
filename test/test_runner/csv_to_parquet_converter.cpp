...
arrow::Status CSVToParquetConverter::RunCSVToParquetConversion(
    ...
    ARROW_ASSIGN_OR_RAISE(auto csvReader,
        arrow::csv::TableReader::Make(arrow::io::default_io_context(), infile, readOptions,
            parseOptions, arrow::csv::ConvertOptions::Defaults()));
    ...
    PARQUET_THROW_NOT_OK(
        parquet::arrow::WriteTable(*csvTable, arrow::default_memory_pool(), outFileStream, csvTable->num_rows()));
...
CSVToParquetConverter::CopyCommandInfo CSVToParquetConverter::createCopyCommandInfo(const std::string& dataset, const std::string& copyStatement) {
    auto tokens = StringUtils::split(copyStatement, " ");
    auto path = std::filesystem::path(tokens[3]);
    removeQuotes(tokens[3]);
...
std::vector<CSVToParquetConverter::CopyCommandInfo> CSVToParquetConverter::readCopyCommandsFromCopyCypherFile(const std::string& dataset) {
    auto copyFile = TestHelper::appendKuzuRootPath(
        "dataset/" + dataset + "/" + TestHelper::COPY_FILE_NAME);
    std::ifstream file(copyFile);
    if (!file.is_open()) {
        throw Exception(StringUtils::string_format(
            "Error opening file: {}, errno: {}.", copyFile, errno));
    }
    std::string line;
...
void CSVToParquetConverter::createCopyFile(const std::string& dataset, const std::vector<CSVToParquetConverter::CopyCommandInfo>& copyCommands) {
...
void CSVToParquetConverter::convertCSVFilesToParquet(const std::vector<CSVToParquetConverter::CopyCommandInfo>& copyCommands) {
    for (auto copyCommand : copyCommands) {
        if (!FileUtils::fileOrPathExists(copyCommand.parquetFilePath)) {
            spdlog::info("CONVERTING: {} to {}", copyCommand.csvFilePath, copyCommand.parquetFilePath);
            RunCSVToParquetConversion(copyCommand.csvFilePath, copyCommand.parquetFilePath, copyCommand.delimiter, copyCommand.csvHasHeader);
        }
    }
}
...
void CSVToParquetConverter::convertCSVDatasetToParquet(std::string& dataset) {
    ...
    std::vector<CSVToParquetConverter::CopyCommandInfo> copyCommands = readCopyCommandsFromCopyCypherFile(dataset);
