...
TEST_F(AggrExpressionEvaluatorTest, CountStarTest) {
    ...
    countFunction->update((uint8_t*)countStarState.get(), nullptr,
    ...
    countFunction->combine((uint8_t*)countStarState.get(), (uint8_t*)otherCountStarState.get());
    countFunction->finalize((uint8_t*)countStarState.get());
...
TEST_F(AggrExpressionEvaluatorTest, CountTest) {
    ...
    auto countState = static_unique_pointer_cast<AggregateState, BaseCountFunction::CountState>(
        countFunction->initialize());
    countFunction->update((uint8_t*)countState.get(), int64ValueVector.get(), 1 /* multiplicity */);
    auto otherCountState =
        static_unique_pointer_cast<AggregateState, BaseCountFunction::CountState>(
            countFunction->initialize());
    otherCountState->val = 10;
    countFunction->combine((uint8_t*)countState.get(), (uint8_t*)otherCountState.get());
    countFunction->finalize((uint8_t*)countState.get());
...
TEST_F(AggrExpressionEvaluatorTest, INT64SumTest) {
    ...
    auto sumState = static_unique_pointer_cast<AggregateState, SumFunction<int64_t>::SumState>(
        sumFunction->initialize());
    sumFunction->update((uint8_t*)sumState.get(), int64ValueVector.get(), 1 /* multiplicity */);
    ...
    sumFunction->combine((uint8_t*)sumState.get(), (uint8_t*)otherSumState.get());
    sumFunction->finalize((uint8_t*)sumState.get());
...
TEST_F(AggrExpressionEvaluatorTest, DOUBLESumTest) {
    ...
    auto sumState = static_unique_pointer_cast<AggregateState, SumFunction<double_t>::SumState>(
        sumFunction->initialize());
    sumFunction->update((uint8_t*)sumState.get(), doubleValueVector.get(), 1 /* multiplicity */);
    auto otherSumState =
...
TEST_F(AggrExpressionEvaluatorTest, UNSTRSumTest) {
    ...
    auto sumState = static_unique_pointer_cast<AggregateState, SumFunction<Value>::SumState>(
        sumFunction->initialize());
    sumFunction->update((uint8_t*)sumState.get(), unStrValueVector.get(), 1 /* multiplicity */);
...
TEST_F(AggrExpressionEvaluatorTest, INT64AvgTest) {
    ...
    auto avgState = static_unique_pointer_cast<AggregateState, AvgFunction<int64_t>::AvgState>(
        avgFunction->initialize());
    avgFunction->update((uint8_t*)avgState.get(), int64ValueVector.get(), 1 /* multiplicity */);
    ...
    avgFunction->combine((uint8_t*)avgState.get(), (uint8_t*)otherAvgState.get());
    avgFunction->finalize((uint8_t*)avgState.get());
    auto sumValue = 10;
    for (auto i = 0u; i < 100; i++) {
        if (i % 2 != 0) {
            sumValue += i;
...
TEST_F(AggrExpressionEvaluatorTest, DOUBLEAvgTest) {
    ...
    auto avgState = static_unique_pointer_cast<AggregateState, AvgFunction<double_t>::AvgState>(
        avgFunction->initialize());
    avgFunction->update((uint8_t*)avgState.get(), doubleValueVector.get(), 1 /* multiplicity */);
    auto otherAvgState =
...
TEST_F(AggrExpressionEvaluatorTest, INT64MaxTest) {
    ...
    auto maxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<int64_t>::MinMaxState>(
            maxFunction->initialize());
    maxFunction->update((uint8_t*)maxState.get(), int64ValueVector.get(), 1 /* multiplicity */);
    auto otherMaxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<int64_t>::MinMaxState>(
            maxFunction->initialize());
    otherMaxState->val = 101;
    otherMaxState->isNull = false;
    maxFunction->combine((uint8_t*)maxState.get(), (uint8_t*)otherMaxState.get());
    maxFunction->finalize((uint8_t*)maxState.get());
...
TEST_F(AggrExpressionEvaluatorTest, DOUBLEMaxTest) {
    ...
    auto maxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<double_t>::MinMaxState>(
            maxFunction->initialize());
    maxFunction->update((uint8_t*)maxState.get(), doubleValueVector.get(), 1 /* multiplicity */);
    auto otherMaxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<double_t>::MinMaxState>(
            maxFunction->initialize());
...
TEST_F(AggrExpressionEvaluatorTest, STRINGMaxTest) {
    ...
    auto maxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<gf_string_t>::MinMaxState>(
            maxFunction->initialize());
    maxFunction->update((uint8_t*)maxState.get(), stringValueVector.get(), 1 /* multiplicity */);
    auto otherMaxState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<gf_string_t>::MinMaxState>(
            maxFunction->initialize());
...
TEST_F(AggrExpressionEvaluatorTest, UNSTRMaxTest) {
    ...
    auto maxState = static_unique_pointer_cast<AggregateState, MinMaxFunction<Value>::MinMaxState>(
        maxFunction->initialize());
    maxFunction->update((uint8_t*)maxState.get(), unStrValueVector.get(), 1 /* multiplicity */);
    auto otherMaxState =
...
TEST_F(AggrExpressionEvaluatorTest, INT64MinTest) {
    ...
    auto minState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<int64_t>::MinMaxState>(
            minFunction->initialize());
    minFunction->update((uint8_t*)minState.get(), int64ValueVector.get(), 1 /* multiplicity */);
    auto otherMinState =
        static_unique_pointer_cast<AggregateState, MinMaxFunction<int64_t>::MinMaxState>(
            minFunction->initialize());
    otherMinState->val = -10;
    otherMinState->isNull = false;
    minFunction->combine((uint8_t*)minState.get(), (uint8_t*)otherMinState.get());
    minFunction->finalize((uint8_t*)minState.get());
