...
std::string OllamaEmbedding::getPath(const std::string&) const { return "/api/embeddings"; }
httplib::Headers OllamaEmbedding::getHeaders() const
{
    return httplib::Headers{{"Content-Type", "application/json"}};
}
nlohmann::json OllamaEmbedding::getPayload(const std::string& model, const std::string& text) const
{
...
{
    static const std::unordered_map<std::string, uint64_t> modelDimensionMap = {{"nomic-embed-text", 768}, {"all-minilm:l6-v2", 384}};
