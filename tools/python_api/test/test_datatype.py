...
def test_bool_wrap(establish_connection):
    ...
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.isStudent;")
...
def test_int_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.age;")
    assert result.hasNext()
...
def test_double_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.eyeSight;")
    assert result.hasNext()
...
def test_date_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.birthdate;")
    assert result.hasNext()
...
def test_timestamp_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.registerTime;")
    assert result.hasNext()
...
def test_interval_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.lastJobDuration;")
    assert result.hasNext()
...
def test_list_wrap(establish_connection):
    conn, db = establish_connection
    result = conn.query("MATCH (a:person) WHERE a.ID = 0 RETURN a.courseScoresPerTerm;")
    assert result.hasNext()
