...
def validate_scan_pandas_results(results):
    assert results.get_next() == [True, 1, 10, 100, 1000, -1, -10, -100, -1000, -0.5199999809265137, 5132.12321,
                                  ...
                                  datetime.timedelta(microseconds=500), None, [], 528, 3.562, ['Alice', None],
                                  datetime.date(1996, 2, 15), '12331']
    assert results.get_next() == [False, 2, 20, 200, 2000, -2, -20, -200, -2000, None, 24.222,
                                  ...
                                  datetime.datetime(1981, 11, 13, 22, 2, 52),
                                  datetime.timedelta(seconds=1), 'Ascii only', [40, 20, 10], -9999, 4.213, [],
                                  datetime.date(2013, 2, 22), 'test string']
    assert results.get_next() == [None, 3, 30, 300, 3000, -3, -30, -300, -3000, -3.299999952316284, None,
                                  ...
                                  datetime.timedelta(seconds=2, milliseconds=500), 'ñ中国字', [30, None], None, None,
                                  None, datetime.date(2055, 1, 14), '5.623']
    assert results.get_next() == [False, 4, 40, 400, 4000, -4, -40, -400, -4000, 4.400000095367432, 4.444,
                                  ...
                                  datetime.datetime(2008, 1, 11, 22, 10, 3),
                                  datetime.timedelta(seconds=3, milliseconds=22), '😂', None, 56677, 67.13,
                                  ['Dan, Ella', 'George'], datetime.date(2018, 3, 17),
                                  None]
...
def test_scan_pandas(get_tmp_path):
    ...
    data = {
        ...
        'past_date': np.array([datetime.date(1996, 2, 15), datetime.date(2013, 2, 22),
                               datetime.date(2055, 1, 14), datetime.date(2018, 3, 17)], dtype=object),
...
def test_scan_pandas_timestamp(get_tmp_path):
    db = kuzu.Database(get_tmp_path)
    conn = kuzu.Connection(db)
    ts = np.array(
        [datetime.datetime(1996, 2, 15, hour=12, minute=22, second=54),
         ...
         None,
         datetime.datetime(2033, 2, 11, microsecond=55)], dtype='object')
    df = pd.DataFrame({'timestamp': ts})
    # Pandas will automatically convert the column from object to timestamp, so we need to manually cast back to object.
...
def test_int64_overflow(get_tmp_path):
    db = kuzu.Database(get_tmp_path)
    conn = kuzu.Connection(db)
...
def test_scan_all_null(get_tmp_path):
    db = kuzu.Database(get_tmp_path)
    conn = kuzu.Connection(db)
    data = {
        'id': np.array([None, None, None], dtype=object)
    }
    df = pd.DataFrame(data)
