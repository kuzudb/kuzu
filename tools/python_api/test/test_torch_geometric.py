...
def test_to_torch_geometric_heterogeneous_graph(establish_connection):
    ...
    for i in range(4):
        assert ground_truth.TINY_SNB_PERSONS_GROUND_TRUTH[pos_to_idx['person'][i]]['eyeSight'] - \
...
def test_to_torch_geometric_no_properties_converted(establish_connection):
    ...
    for i in range(1):
        ...
        assert src != dst
        assert pos_to_idx['personLongString'][dst] in ground_truth.PERSONLONGSTRING_KNOWS_GROUND_TRUTH[pos_to_idx['personLongString'][src]]
    ...
    for i in range(2):
        assert ground_truth.PERSONLONGSTRING_GROUND_TRUTH[pos_to_idx['personLongString'][i]
