...
int main(int argc, char **argv) {
    ...
    std::string basePath = input.getCmdOption("-basePath");
    std::string benchSuffix = input.getCmdOption("-benchSuffix");
    int warmupTimes = stoi(input.getCmdOption("-warmup"));
    int isCreateSchema = stoi(input.getCmdOption("-createSchema"));
    int k = stoi(input.getCmdOption("-k"));
    int skipRecall = stoi(input.getCmdOption("-skipRecall"));
...
//                baseBenchPath + "/queries_0.50.sql"
    ...
    for (auto &queriesPath: testQueries) {
        ...
        vector_id_t *gtVecs = nullptr;
        if (!skipRecall) {
            gtVecs = new vector_id_t[queryNumVectors * k];
