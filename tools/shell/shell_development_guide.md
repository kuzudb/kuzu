...
After every character is processed, the `refreshLine()` function is called to update what the user sees on the screen. For `:singleline`, this line refresh starts off with truncating the current buffer so it can fit in a single line on the screen. It truncates so the cursor is always on the screen. As well, while it is truncating, it adds highlighting escape codes to the buffer.
