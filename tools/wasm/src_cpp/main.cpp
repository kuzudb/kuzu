...
Value valueFromEmscriptenValue(const val& value) {
    ...
    if (type == "number") {
        ...
        auto doubleValue = value.as<double>();
        if (doubleValue > maxSafeInteger || doubleValue < minSafeInteger) {
            return Value(doubleValue);
        }
        auto intValue = value.as<int64_t>();
        return intValue == doubleValue ? Value(intValue) : Value(doubleValue);
    }
    if (type == "string") {
...
val valueGetAsEmscriptenValue(const Value& value) {
    ...
    case LogicalTypeID::BLOB: {
        ...
        return val(typed_memory_view(blobVal.size(), (const char*)blobVal.c_str()));
    }
