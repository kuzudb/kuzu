...
JNIEXPORT jboolean JNICALL Java_com_kuzu_db_Native_kuzu_1has_1next_1query_1result(JNIEnv* env,
    jclass, jobject thisQR) {
    QueryResult* qr = getQueryResult(env, thisQR);
    return qr->hasNextQueryResult();
}

JNIEXPORT jobject JNICALL Java_com_kuzu_db_Native_kuzu_1get_1next_1query_1result(JNIEnv* env,
    jclass, jobject thisQR) {
    QueryResult* qr = getQueryResult(env, thisQR);
